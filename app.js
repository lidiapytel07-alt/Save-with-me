...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    navigator.serviceWorker.register("sw.js").catch(() => {
      updateSyncStatus("offline niedostepny");
    });
  });
}
...
async function syncFromCloud(options = {}) {
  ...
  try {
    ...
    const response = await fetch(buildSupabaseUrl("select"), {
      headers: supabaseHeaders({ noCache: true }),
    });
    ...
    if (remoteData && Array.isArray(remoteData.items)) {
      ...
      saveState();
      render();
      updateSyncStatus(`pobrano ${pluralize(remoteData.items.length)}`);
      return;
    }
...
async function syncToCloud(options = {}) {
  ...
  try {
    ...
    updateSyncStatus(`wyslano ${pluralize(state.items.length)}`);
  } catch (error) {
    updateSyncStatus(error.message || "blad sync");
  }
}

...
syncForm.addEventListener("submit", async (event) => {
  ...
  saveState();
  await syncFromCloud({ pushIfEmpty: true });
  startSyncTimer();
});

document.querySelector("#pullButton").addEventListener("click", () => syncFromCloud({ pushIfEmpty: false }));
document.querySelector("#pushButton").addEventListener("click", () => syncToCloud());
...
startSyncTimer();
if (hasSyncConfig()) {
  syncFromCloud({ pushIfEmpty: true });
}
...
function renderList(items) {
  ...
  items.forEach((item) => {
    ...
    row.querySelector("strong").textContent = formatMoney.format(item.amount);
    list.append(row);
  });
}
...
async function syncToCloud(options = {}) {
  ...
  try {
    ...
    if (!response.ok) throw new Error("Nie udalo sie wyslac danych.");
    updateSyncStatus("zsynchronizowano");
  } catch {
    updateSyncStatus("blad sync");
  }
